...
{
	global $conn;
	$statement=$conn->prepare('SELECT * FROM items WHERE '.$where.'=? ORDER BY '.$where.' DESC');
    $statement->execute(array($value));
    $result=$statement->fetchAll();
    return $result;
...
/*
**chect user status function v1.0
**gets number of row with userstatus 1 from database
*/

function checkUserStatus($user)
{
   	global $conn;
	$statement=$conn->prepare('SELECT username,reg_status FROM users WHERE reg_status=0 AND username=?');
